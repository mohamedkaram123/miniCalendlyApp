...
export default function Addroles({ show, handleClose,handleSave, trans, roles }) {

  const [btnSave, setbtnSave] = useState(false)
  const [roles_arrData, setroles_arrData] = useState([])
  ...
  const andleSaveChange = () => {
    ...
    post(Urls.static + "create_role", rolesData, res => {
      if (res.status == 1) {
        handleSave()
                setbtnSave(false);
    ...
    return (
      <div >
        ...
        <Modal  backdrop="static" scrollable={true}  size='lg' show={show} onHide={handleClose}>
          <Modal.Header >
            <Modal.Title>{trans("Add roles")}</Modal.Title>
          </Modal.Header>
